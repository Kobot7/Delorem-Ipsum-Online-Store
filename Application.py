...
def home():
    ...
    try:
        ...
        print("Yo home page ain't givin me prods")
    prods = []
    for serial_no in productsDict:
        prods.append(productsDict[serial_no])
    db.close()
    viewsList = sort_by(prods, 'view', 'descending')[:5]
    purchasesList = sort_by(prods, 'purchase', 'descending')[:5]
    List = viewsList + purchasesList
    carouselStars = []
    for i in range(5):
        select = random.randint(0,9-i)
        carouselStars.append(List[select])
        List.pop(select)
    first = carouselStars[1]
    carouselStars.pop(1)
    searchForm = searchBar()
    if request.method == "POST" and searchForm.validate():
        return redirect('/search/' + searchForm.search_input.data)

    return render_template("home.html", current=current, searchForm=searchForm, viewsList = viewsList, purchasesList = purchasesList ,carouselList = carouselStars,first=first)
